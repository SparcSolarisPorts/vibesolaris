...
int vs_set_proxy(VSContext *c,const char *value){
    const char *v=value?value:"";
    if(!c)return -1;
    if(!strcmp(v,"off")||!strcmp(v,"none")||!strcmp(v,"disable")||!strcmp(v,"disabled")){
        c->proxy_enabled=0;(void)vs_persist_settings(c);return 0;
    }
    if(!strcmp(v,"on")||!strcmp(v,"enable")||!strcmp(v,"enabled")){
        if(!c->proxy[0])return -1;
        c->proxy_enabled=1;(void)vs_persist_settings(c);return 0;
    }
    if(!v[0]){c->proxy_enabled=0;c->proxy[0]=0;(void)vs_persist_settings(c);return 0;}
    /* Accepted forms are host:port and user:pass@host:port.  A URI scheme is
       also tolerated because libcurl accepts it, but is not required. */
    if(!strchr(v,':'))return -1;
    copy_value(c->proxy,sizeof(c->proxy),v);c->proxy_enabled=1;(void)vs_persist_settings(c);return 0;
}

void vs_proxy_redacted(const VSContext *c,char *out,size_t cap){
    const char *v,*at,*cred,*colon,*scheme;size_t pre,usern,hostn,pos=0;
    if(!out||cap==0)return;
    out[0]=0;
    if(!c||!c->proxy_enabled||!c->proxy[0]){copy_value(out,cap,"Off");return;}
    v=c->proxy;scheme=strstr(v,"://");cred=scheme?scheme+3:v;at=strrchr(cred,'@');
    if(!at){copy_value(out,cap,v);return;}
    pre=(size_t)(cred-v);if(pre>=cap)pre=cap-1;if(pre){memcpy(out,v,pre);pos=pre;out[pos]=0;}
    colon=(const char*)memchr(cred,':',(size_t)(at-cred));
    if(colon){usern=(size_t)(colon-cred);if(pos+usern+1<cap){memcpy(out+pos,cred,usern);pos+=usern;out[pos++]=':';}if(pos+4<cap){memcpy(out+pos,"****",4);pos+=4;}if(pos+1<cap)out[pos++]='@';}
    else {if(pos+4<cap){memcpy(out+pos,"****",4);pos+=4;}if(pos+1<cap)out[pos++]='@';}
    hostn=strlen(at+1);if(pos+hostn>=cap)hostn=cap-pos-1;memcpy(out+pos,at+1,hostn);pos+=hostn;out[pos]=0;
}
...
char *vs_config_serialize(const VSContext *cc){VSContext *c=(VSContext*)cc;char *buf=NULL;size_t len=0,cap=0;int i;char key[128],val[64];if(!c)return NULL;save_active(c);if(append_line(&buf,&len,&cap,"provider",c->provider.name)||append_line(&buf,&len,&cap,"protocol",vs_protocol_name(c->provider.protocol))||append_line(&buf,&len,&cap,"base_url",c->provider.base_url)||append_line(&buf,&len,&cap,"model",c->provider.model))goto fail;snprintf(val,sizeof(val),"%d",c->cache_enabled?1:0);if(append_line(&buf,&len,&cap,"cache",val))goto fail;snprintf(val,sizeof(val),"%d",c->compact_disabled?0:1);if(append_line(&buf,&len,&cap,"autocompact",val))goto fail;snprintf(val,sizeof(val),"%d",c->proxy_enabled?1:0);if(append_line(&buf,&len,&cap,"proxy_enabled",val)||append_line(&buf,&len,&cap,"proxy",c->proxy)||append_line(&buf,&len,&cap,"oauth_client_id",c->oauth.client_id)||append_line(&buf,&len,&cap,"oauth_authorize_url",c->oauth.authorize_url)||append_line(&buf,&len,&cap,"oauth_token_url",c->oauth.token_url)||append_line(&buf,&len,&cap,"oauth_scopes",c->oauth.scopes)||append_line(&buf,&len,&cap,"oauth_redirect_uri",c->oauth.redirect_uri)||append_line(&buf,&len,&cap,"oauth_access_token",c->oauth.access_token)||append_line(&buf,&len,&cap,"oauth_refresh_token",c->oauth.refresh_token)||append_line(&buf,&len,&cap,"oauth_token_type",c->oauth.token_type))goto fail;snprintf(val,sizeof(val),"%ld",c->oauth.expires_at);if(append_line(&buf,&len,&cap,"oauth_expires_at",val))goto fail;
