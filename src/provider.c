...
char *vs_build_system_prompt(const VSContext *c){
    ...
    char *mcp=vs_mcp_prompt_fragment(c);size_t n=strlen(base)+strlen(c->agent_md)+strlen(c->os_name)+strlen(c->os_release)+strlen(c->arch)+strlen(c->cwd)+(mcp?strlen(mcp):0)+2600;char *o=(char*)malloc(n);
    if(!o){if(mcp)free(mcp);return NULL;}
    snprintf(o,n,
        "%sHost OS: %s %s\nCPU architecture: %s\nWorking directory: %s\n\n"
        "AGENT.MD:\n%s\n\nMCP TOOLS:\n%s\n\n"
        "REASONING AND LOOP DISCIPLINE (follow strictly):\n"
        "- Work in short direct cycles: inspect, edit, run, observe, decide. Prefer one concrete action over long deliberation.\n"
        "- Do not restate the task, your plan, or earlier results. Do not re-derive facts you already established.\n"
        "- Never repeat an action that already produced a result; if something failed twice, change approach instead of retrying it.\n"
        "- Keep replies short. A tool directive is progress; paragraphs of speculation are not.\n"
        "- Stop as soon as the requested work is done: emit [[VS_FINAL]] with a brief result. Do not redo finished work as a double-check.\n"
        "- If genuinely blocked, emit VS_NEED_USER with your question immediately instead of circling.\n"
        "- Older turns may appear as a [Compacted earlier conversation summary]; treat it as established context and never ask the user to repeat it.\n"
        "- When the user asks you to hurry, reduce explanation and act.\n",
